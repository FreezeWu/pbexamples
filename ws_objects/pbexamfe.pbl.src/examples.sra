...
$PBExportComments$PowerBuilder Code Examples のアプリケーション オブジェクト
forward
...
if not f_create_external_func_uo(gu_ext_func) then
	Messagebox ("外部関数ユーザ オブジェクト","初期化中にエラーが発生しました。~n~r "+&
					"このセッションの中で外部関数を呼び出すのは適切ではありません。")
end if
...
if li_rc = -1 then
	...
	f_set_profile("sqlca", "dbparm", "ConnectString='DSN=PB Demo DB V2017R2;UID=dba;PWD=sql'" )
else
