...
FUNCTION ulong SetCurrentDirectoryA( ref string currentdir ) LIBRARY "Kernel32.dll" alias for "SetCurrentDirectoryA;Ansi"

end prototypes

type variables
string is_labelfile = "pblab126.ini"
end variables
...
If li_file_rc = -1 Then
	If MessageBox ( "ラベルファイル", "'" + is_labelfile + "'" + &
						" がみつかりません。~n~r~n~r次の画面でファイルの場所を指定できます。" + &
						"~n~r~n~rすぐにファイルを指定しますか？", Exclamation!, YesNo!, 2)  = 2 then
		Return -1
	Else
		// Allow the user to find the directory
		if GetFileOpenName("Find " + is_labelfile, ls_Path, is_labelfile ) = 1 then
			li_file_rc = FileOpen ( is_labelfile )
			If li_file_rc = -1 Then
				Messagebox("ラベル INI ファイルがみつかりません","'" + &
				is_labelfile + "' は普通、PB のランタイムディレクトリにあります。",Exclamation!)
...
//based on the pblab126.ini file.
